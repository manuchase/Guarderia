...
import {
  ...
  MessageSquare, ChevronLeft, ChevronRight, ChevronDown, ShieldCheck, FileDown
} from "lucide-react";
  ...
  return (
    <div
      ...
      style={{
        ...
        boxShadow: "0 8px 32px rgba(1,180,188,0.10), inset 0 1px 0 rgba(255,255,255,0.9)",
        ...style,
      }}
    >
      {children}
...
function GlassButton({ children, onClick, variant = "primary", className = "", disabled, type = "button" }) {
  const variants = {
    primary: { background: "linear-gradient(135deg, #01B4BC 0%, #029AA1 100%)", color: "white", boxShadow: "0 4px 20px rgba(1,180,188,0.35)" },
    secondary: { background: "rgba(255,255,255,0.7)", color: "#0E7C82", border: "1px solid rgba(1,180,188,0.25)" },
    ghost: { background: "transparent", color: "#2E7D74" },
    danger: { background: "rgba(250,84,87,0.1)", color: "#D33B3E", border: "1px solid rgba(250,84,87,0.25)" },
  };
  return (
    <button
      type={type}
...
function Backdrop({ children }) {
  return (
    <div
      ...
      style={{
        background:
          "radial-gradient(circle at 15% 10%, #E3F5E4 0%, transparent 45%), radial-gradient(circle at 85% 20%, #DFF7F7 0%, transparent 40%), radial-gradient(circle at 50% 100%, #FFF3DE 0%, transparent 45%), #FAFAF5",
      ...
      return (
        <div className="w-full max-w-md px-5 mt-16">
          <GlassCard className="p-8 text-center">
            <p className="font-semibold mb-2" style={{ color: "#D33B3E" }}>Ocurrió un error</p>
    ...
    return (
      <Backdrop>
        <div className="flex-1 flex items-center justify-center min-h-screen">
          <Loader2 className="animate-spin" size={28} color="#029AA1" />
        </div>
      </Backdrop>
    );
  }
    ...
    <Backdrop>
      <div className="w-full max-w-3xl px-5 pt-8 pb-4 flex items-center justify-between">
        <div className="flex items-center gap-2.5">
          <Logo size={10} />
          <span className="font-semibold text-lg tracking-tight" style={{ backgroundImage: "linear-gradient(90deg, #5FA55A 0%, #2E7D74 55%, #01B4BC 100%)", WebkitBackgroundClip: "text", backgroundClip: "text", color: "transparent" }}>
            Skillmind
          </span>
        </div>
        {currentPro && (
      ...
      <div className="w-full max-w-3xl px-5 pb-24">
        <ErrorBoundary>
          ...
          {view === "dashboard" && currentPro && (
            <AdminDashboard pro={currentPro} professionals={professionals} setProfessionals={setProfessionals} showToast={showToast} onPreviewMaestra={(code) => setMaestraCode(code)} />
          )}
        </ErrorBoundary>
      </div>
      <Toast msg={toast} />
  ...
  return (
    <div className="mt-10 flex flex-col items-center">
      <GlassCard className="w-full p-8">
        <h1 className="text-2xl font-semibold mb-1" style={{ color: "#1F3A35" }}>Iniciar sesión</h1>
        <p className="text-sm mb-6" style={{ color: "#6B7280" }}>Ingresa con el usuario y contraseña que te asignaron.</p>
        <div className="space-y-4">
          <div>
            <label className="text-xs font-medium mb-1.5 block" style={{ color: "#2E7D74" }}>Usuario</label>
            <input className="w-full px-4 py-3 rounded-2xl text-sm outline-none" style={{ background: "rgba(255,255,255,0.7)", border: "1px solid rgba(1,180,188,0.2)" }} placeholder="Tu usuario" autoCapitalize="none" value={username} onChange={(e) => setUsername(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") submit(e); }} />
          </div>
          <div>
            <label className="text-xs font-medium mb-1.5 block" style={{ color: "#2E7D74" }}>Contraseña</label>
            <input type="password" className="w-full px-4 py-3 rounded-2xl text-sm outline-none" style={{ background: "rgba(255,255,255,0.7)", border: "1px solid rgba(1,180,188,0.2)" }} placeholder="Tu contraseña" value={password} onChange={(e) => setPassword(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") submit(e); }} />
          </div>
          {error && <p className="text-sm" style={{ color: "#D33B3E" }}>{error}</p>}
...
function HomeTile({ icon: Icon, label, sublabel, onClick, accent }) {
  return (
    <button onClick={onClick} className="flex flex-col items-start gap-3 p-5 rounded-3xl text-left transition-all active:scale-95"
      style={{ background: "rgba(255,255,255,0.6)", backdropFilter: "blur(20px)", WebkitBackdropFilter: "blur(20px)", border: "1px solid rgba(255,255,255,0.8)", boxShadow: "0 8px 32px rgba(1,180,188,0.10), inset 0 1px 0 rgba(255,255,255,0.9)" }}>
      ...
      <div><p className="font-semibold" style={{ color: "#1F3A35" }}>{label}</p>{sublabel && <p className="text-xs mt-0.5" style={{ color: "#9CA3AF" }}>{sublabel}</p>}</div>
    </button>
  );
}

function AdminDashboard({ pro, professionals, setProfessionals, showToast, onPreviewMaestra }) {
  ...
  if (section === "home") {
    return (
      <div className="mt-2 space-y-5">
        <div>
          <h1 className="text-xl font-semibold mb-0.5" style={{ color: "#1F3A35" }}>{pro.guarderia || `Hola, ${pro.name}`}</h1>
          <p className="text-sm" style={{ color: "#9CA3AF" }}>{pro.guarderia ? `Hola, ${pro.name}` : "¿Qué quieres revisar hoy?"}</p>
        </div>
        <div className="grid grid-cols-2 gap-3">
          <HomeTile icon={Users} label="Maestras" sublabel={`${maestras.length} registrada(s)`} accent="linear-gradient(135deg,#01B4BC,#029AA1)" onClick={() => setSection("maestras")} />
          <HomeTile icon={Baby} label="Niños" sublabel={`${ninos.length} registrado(s)`} accent="linear-gradient(135deg,#FA8925,#C96A0F)" onClick={() => setSection("ninos")} />
          <HomeTile icon={Utensils} label="Bitácoras diarias" sublabel="Reportes de hoy" accent="linear-gradient(135deg,#5FA55A,#3F7A45)" onClick={() => setSection("bitacoras")} />
          {pro.role === "superadmin" && (
            <HomeTile icon={ShieldCheck} label="Usuarios" sublabel={`${professionals.length} cuenta(s)`} accent="linear-gradient(135deg,#FA5457,#D33B3E)" onClick={() => setSection("usuarios")} />
          )}
        </div>
      </div>
    );
  ...
  const titles = { maestras: "Maestras", ninos: "Niños", bitacoras: "Bitácoras diarias", usuarios: "Usuarios" };
  ...
  return (
    <div className="mt-2 space-y-4">
      ...
      <h1 className="text-xl font-semibold" style={{ color: "#1F3A35" }}>{titles[section]}</h1>
  ...
  return (
    <div className="space-y-4">
      <GlassCard className="p-6 space-y-3">
        <h2 className="font-semibold" style={{ color: "#1F3A35" }}>Nuevo usuario</h2>
        <input className="w-full px-4 py-3 rounded-2xl text-sm outline-none" style={{ background: "rgba(255,255,255,0.7)", border: "1px solid rgba(1,180,188,0.2)" }} placeholder="Usuario" autoCapitalize="none" value={username} onChange={(e) => setUsername(e.target.value)} />
        <input type="password" className="w-full px-4 py-3 rounded-2xl text-sm outline-none" style={{ background: "rgba(255,255,255,0.7)", border: "1px solid rgba(1,180,188,0.2)" }} placeholder="Contraseña" value={password} onChange={(e) => setPassword(e.target.value)} />
        <input className="w-full px-4 py-3 rounded-2xl text-sm outline-none" style={{ background: "rgba(255,255,255,0.7)", border: "1px solid rgba(1,180,188,0.2)" }} placeholder="Nombre de guardería" value={guarderia} onChange={(e) => setGuarderia(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") addUser(); }} />
        {formError && <p className="text-sm" style={{ color: "#D33B3E" }}>{formError}</p>}
      ...
      <GlassCard className="p-2">
        <div className="px-3 py-3 space-y-2">
          {others.length === 0 && <p className="text-sm text-center py-8" style={{ color: "#9CA3AF" }}>Aún no hay usuarios adicionales.</p>}
          {others.map((u) => (
            <div key={u.id} className="flex items-center justify-between gap-3 p-3.5 rounded-2xl" style={{ background: "rgba(255,255,255,0.6)" }}>
              <div className="min-w-0">
                <p className="text-sm font-medium truncate" style={{ color: "#1F3A35" }}>{u.guarderia || u.name}</p>
                <p className="text-xs" style={{ color: "#9CA3AF" }}>@{u.username}</p>
              </div>
              <button onClick={() => remove(u)} className="p-2.5 rounded-xl shrink-0" style={{ background: "rgba(250,84,87,0.08)" }}><Trash2 size={15} color="#D33B3E" /></button>
            </div>
          ))}
        </div>
      </GlassCard>
...
function MaestrasTab({ pro, maestras, loading, onAdded, onRemoved, showToast }) {
  ...
  return (
    <div className="space-y-4">
      <GlassCard className="p-6 space-y-3">
        <h2 className="font-semibold" style={{ color: "#1F3A35" }}>Nueva maestra</h2>
        <input className="w-full px-4 py-3 rounded-2xl text-sm outline-none" style={{ background: "rgba(255,255,255,0.7)", border: "1px solid rgba(1,180,188,0.2)" }} placeholder="Nombre completo *" value={name} onChange={(e) => setName(e.target.value)} />
        <input className="w-full px-4 py-3 rounded-2xl text-sm outline-none" style={{ background: "rgba(255,255,255,0.7)", border: "1px solid rgba(1,180,188,0.2)" }} placeholder="Función (ej. Maestra titular, Auxiliar)" value={funcion} onChange={(e) => setFuncion(e.target.value)} />
        <input className="w-full px-4 py-3 rounded-2xl text-sm outline-none" style={{ background: "rgba(255,255,255,0.7)", border: "1px solid rgba(1,180,188,0.2)" }} placeholder="Grupo (ej. Maternal, Lactantes 1) *" value={grupo} onChange={(e) => setGrupo(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") addMaestra(); }} />
        {formError && <p className="text-sm" style={{ color: "#D33B3E" }}>{formError}</p>}
        ...
        <div className="px-3 py-3 space-y-2">
          ...
          {maestras.map((m) => (
            <div key={m.code} className="flex items-center justify-between gap-3 p-3.5 rounded-2xl" style={{ background: "rgba(255,255,255,0.6)" }}>
              <div className="min-w-0">
                <p className="text-sm font-medium truncate" style={{ color: "#1F3A35" }}>{m.name}</p>
                <p className="text-xs" style={{ color: "#9CA3AF" }}>{m.funcion} · {m.grupo}</p>
              </div>
              <button onClick={() => remove(m.code)} className="p-2.5 rounded-xl shrink-0" style={{ background: "rgba(250,84,87,0.08)" }}><Trash2 size={15} color="#D33B3E" /></button>
            </div>
          ))}
        </div>
      </GlassCard>
...
function NinosTab({ pro, ninos, maestras, loading, onAdded, onRemoved, showToast }) {
  ...
  return (
    <div className="space-y-4">
      <GlassCard className="p-6 space-y-3">
        <h2 className="font-semibold" style={{ color: "#1F3A35" }}>Nuevo niño</h2>
        <input className="w-full px-4 py-3 rounded-2xl text-sm outline-none" style={{ background: "rgba(255,255,255,0.7)", border: "1px solid rgba(1,180,188,0.2)" }} placeholder="Nombre" value={name} onChange={(e) => setName(e.target.value)} />
        <input className="w-full px-4 py-3 rounded-2xl text-sm outline-none" list="grupos-list" style={{ background: "rgba(255,255,255,0.7)", border: "1px solid rgba(1,180,188,0.2)" }} placeholder="Grupo (ej. Maternal)" value={grupo} onChange={(e) => setGrupo(e.target.value)} />
        <datalist id="grupos-list">{grupoOptions.map((g) => <option key={g} value={g} />)}</datalist>
        <div className="flex items-center gap-3">
          <div className="w-14 h-14 rounded-2xl overflow-hidden flex items-center justify-center shrink-0" style={{ background: "rgba(250,137,37,0.12)" }}>
            {processingPhoto ? <Loader2 className="animate-spin" size={18} color="#C96A0F" /> : foto ? <img src={foto} alt="" className="w-full h-full object-cover" /> : <Baby size={20} color="#C96A0F" />}
          </div>
          <label className="flex-1">
            <input type="file" accept="image/*" className="hidden" onChange={handlePhotoUpload} />
            <span className="block text-center px-4 py-3 rounded-2xl text-sm font-medium cursor-pointer" style={{ background: "rgba(255,255,255,0.7)", border: "1px solid rgba(1,180,188,0.2)", color: "#0E7C82" }}>
              {foto ? "Cambiar foto" : "Subir foto"}
            </span>
          </label>
          {foto && (
            <button onClick={() => setFoto("")} className="p-3 rounded-2xl shrink-0" style={{ background: "rgba(250,84,87,0.08)" }}><X size={15} color="#D33B3E" /></button>
          )}
        </div>
          ...
          {ninos.map((n) => (
            <div key={n.id} className="flex items-center justify-between gap-3 p-3.5 rounded-2xl" style={{ background: "rgba(255,255,255,0.6)" }}>
              <div className="flex items-center gap-3 min-w-0">
                <div className="w-9 h-9 rounded-full flex items-center justify-center shrink-0 overflow-hidden" style={{ background: "rgba(250,137,37,0.12)" }}>
                  {n.foto ? <img src={n.foto} alt="" className="w-full h-full object-cover" /> : <Baby size={16} color="#C96A0F" />}
                </div>
                <div className="min-w-0"><p className="text-sm font-medium truncate" style={{ color: "#1F3A35" }}>{n.name}</p><p className="text-xs" style={{ color: "#9CA3AF" }}>{n.grupo}</p></div>
              </div>
              <button onClick={() => remove(n.id)} className="p-2.5 rounded-xl shrink-0" style={{ background: "rgba(250,84,87,0.08)" }}><Trash2 size={15} color="#D33B3E" /></button>
            </div>
          ))}
        </div>
      </GlassCard>
...
function DayNav({ date, setDate }) {
  ...
  return (
    <div className="flex items-center justify-between mb-1">
      <button onClick={() => setDate(dateKey(addDays(d, -1)))} className="p-2 rounded-xl" style={{ background: "rgba(1,180,188,0.08)" }}><ChevronLeft size={16} color="#029AA1" /></button>
      <p className="text-sm font-medium capitalize" style={{ color: "#1F3A35" }}>{dayLabel(d)}</p>
      <button onClick={() => setDate(dateKey(addDays(d, 1)))} className="p-2 rounded-xl" style={{ background: "rgba(1,180,188,0.08)" }}><ChevronRight size={16} color="#029AA1" /></button>
    </div>
  );
}
  ...
  return (
    <div className="rounded-2xl overflow-hidden" style={{ background: "rgba(255,255,255,0.6)" }}>
      <button onClick={() => setExpanded((v) => !v)} className="w-full flex items-center justify-between gap-3 p-3.5">
        <div className="flex items-center gap-3 min-w-0">
          <div className="w-9 h-9 rounded-full flex items-center justify-center shrink-0 overflow-hidden" style={{ background: "rgba(250,137,37,0.12)" }}>
            {nino.foto ? <img src={nino.foto} alt="" className="w-full h-full object-cover" /> : <Baby size={16} color="#C96A0F" />}
          </div>
          <div className="min-w-0 text-left">
            <p className="text-sm font-medium truncate" style={{ color: "#1F3A35" }}>{nino.name}</p>
            <p className="text-xs truncate" style={{ color: "#9CA3AF" }}>
              {lastMeal ? `Comida: ${MEAL_OPTIONS.find((o) => o.key === lastMeal.cantidad)?.label}` : "Sin registros hoy"}
            </p>
          </div>
        </div>
        <ChevronDown size={16} color="#BFE0D6" className="shrink-0 transition-transform" style={{ transform: expanded ? "rotate(180deg)" : "none" }} />
      </button>

      {expanded && log && (
        <div className="px-3.5 pb-4 space-y-4" style={{ borderTop: "1px solid rgba(1,180,188,0.1)" }}>
          {/* Alimentación */}
          <div className="pt-3">
            <p className="text-xs font-medium mb-1.5 flex items-center gap-1.5" style={{ color: "#2E7D74" }}><Utensils size={13} /> Alimentación</p>
            {!readOnly && (
              <div className="grid grid-cols-3 gap-1.5">
                {MEAL_OPTIONS.map((o) => (
                  <button key={o.key} onClick={() => logMeal(o.key)} className="py-2.5 rounded-xl text-xs font-medium" style={{ background: "rgba(95,165,90,0.1)", color: "#3F7A45" }}>{o.label}</button>
                ))}
              </div>
            )}
            <div className="mt-1.5 space-y-0.5">
              {log.alimentacion.length === 0 && <p className="text-xs" style={{ color: "#9FC6BE" }}>Sin registros.</p>}
          ...
          {/* Pañal / baño */}
          <div>
            <p className="text-xs font-medium mb-1.5 flex items-center gap-1.5" style={{ color: "#2E7D74" }}><Droplet size={13} /> Pañal / baño ({diaperCount})</p>
            {!readOnly && (
              <div className="grid grid-cols-2 gap-1.5">
                {DIAPER_OPTIONS.map((o) => (
                  <button key={o.key} onClick={() => logDiaper(o.key)} className="py-2.5 rounded-xl text-xs font-medium" style={{ background: "rgba(1,180,188,0.08)", color: "#0E7C82" }}>{o.label}</button>
                ))}
              </div>
            )}
            <div className="mt-1.5 space-y-0.5">
              {log.panales.length === 0 && <p className="text-xs" style={{ color: "#9FC6BE" }}>Sin registros.</p>}
          ...
          {/* Siesta */}
          <div>
            <p className="text-xs font-medium mb-1.5 flex items-center gap-1.5" style={{ color: "#2E7D74" }}><Moon size={13} /> Siesta</p>
            {!readOnly && (
              napAsking ? (
                <div>
                  <p className="text-xs mb-1.5" style={{ color: "#9CA3AF" }}>¿Cuánto durmió?</p>
                  <div className="grid grid-cols-2 gap-1.5">
                    {NAP_DURATIONS.map((d) => (
                      <button key={d.key} onClick={() => logNapYes(d.key)} className="py-2.5 rounded-xl text-xs font-medium" style={{ background: "rgba(1,180,188,0.08)", color: "#0E7C82" }}>{d.label}</button>
                    ))}
                  </div>
                  <button onClick={() => setNapAsking(false)} className="text-xs mt-1.5" style={{ color: "#9CA3AF" }}>Cancelar</button>
                </div>
              ) : (
                <div>
                  <p className="text-xs mb-1.5" style={{ color: "#9CA3AF" }}>¿Durmió siesta?</p>
                  <div className="grid grid-cols-2 gap-1.5">
                    <button onClick={() => setNapAsking(true)} className="py-2.5 rounded-xl text-xs font-medium" style={{ background: "rgba(95,165,90,0.1)", color: "#3F7A45" }}>Sí</button>
                    <button onClick={logNapNo} className="py-2.5 rounded-xl text-xs font-medium" style={{ background: "rgba(255,255,255,0.7)", color: "#4B5563", border: "1px solid rgba(107,114,128,0.2)" }}>No</button>
                  </div>
                </div>
              )
            )}
            <div className="mt-1.5 space-y-0.5">
              {log.siestas.length === 0 && <p className="text-xs" style={{ color: "#9FC6BE" }}>Sin registros.</p>}
              {log.siestas.map((s, i) => (
          ...
          {/* Ánimo y notas */}
          <div>
            <p className="text-xs font-medium mb-1.5 flex items-center gap-1.5" style={{ color: "#2E7D74" }}><Smile size={13} /> Ánimo y observaciones</p>
            {!readOnly && (
              <div className="grid grid-cols-2 gap-1.5 mb-2">
                {MOOD_OPTIONS.map((o) => (
                  <button key={o.key} onClick={() => logMood(o.key)} className="py-2.5 rounded-xl text-xs font-medium" style={{ background: lastMood?.estado === o.key ? "linear-gradient(135deg,#01B4BC,#029AA1)" : "rgba(255,255,255,0.7)", color: lastMood?.estado === o.key ? "white" : "#0E7C82", border: "1px solid rgba(1,180,188,0.15)" }}>{o.label}</button>
                ))}
              </div>
            )}
            {!readOnly && (
              <div className="flex gap-2 mb-2">
                <input className="flex-1 px-3 py-2.5 rounded-xl text-sm outline-none" style={{ background: "rgba(255,255,255,0.7)", border: "1px solid rgba(1,180,188,0.2)" }} placeholder="Nota rápida…" value={noteText} onChange={(e) => setNoteText(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") sendNote(); }} />
                <button onClick={sendNote} className="p-2.5 rounded-xl" style={{ background: "linear-gradient(135deg,#01B4BC,#029AA1)" }}><Send size={14} color="white" /></button>
              </div>
            )}
            <div className="space-y-0.5">
              {log.animos.length === 0 && log.notas.length === 0 && <p className="text-xs" style={{ color: "#9FC6BE" }}>Sin registros.</p>}
...
function buildBitacoraHTML(maestra, dateStr, entries) {
  ...
  return `
    <div style="font-family:-apple-system,Arial,sans-serif;color:#111;padding:24px;">
      <h1 style="font-size:20px;margin:0 0 2px;">Bitácora diaria — ${maestra.name}</h1>
      <p style="margin:0 0 20px;color:#555;font-size:13px;">${maestra.grupo} · ${fmtDateEs(dateStr)}</p>
  ...
  return (
    <GlassCard className="p-4">
      <button onClick={() => setOpen((v) => !v)} className="w-full flex items-center justify-between gap-3">
        <div className="min-w-0 text-left"><p className="text-sm font-medium" style={{ color: "#1F3A35" }}>{m.name}</p><p className="text-xs" style={{ color: "#9CA3AF" }}>{m.grupo} · {grupoNinos.length} niño(s)</p></div>
        <ChevronDown size={16} color="#BFE0D6" className="shrink-0 transition-transform" style={{ transform: open ? "rotate(180deg)" : "none" }} />
      </button>
      {open && (
        <div className="mt-3 pt-3 space-y-3" style={{ borderTop: "1px solid rgba(1,180,188,0.1)" }}>
          <div className="flex items-center gap-2">
            <button onClick={() => copyLink(m.code)} className="flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-xl text-xs font-medium" style={{ background: "rgba(1,180,188,0.08)", color: "#0E7C82" }}><Copy size={13} /> Copiar link</button>
            <button onClick={() => onPreview(m.code)} className="flex-1 py-2.5 rounded-xl text-xs font-medium" style={{ background: "rgba(255,255,255,0.7)", color: "#2E7D74", border: "1px solid rgba(1,180,188,0.15)" }}>Vista previa</button>
          </div>
...
function MaestraHub({ code, onExitDemo, showToast }) {
  ...
  if (loading) return <div className="flex-1 flex items-center justify-center py-24 min-h-screen"><Loader2 className="animate-spin" color="#029AA1" /></div>;

  if (!maestra) {
    return (
      <div className="w-full max-w-md px-5 mt-16">
        <GlassCard className="p-8 text-center"><p style={{ color: "#1F3A35" }}>Este link no es válido.</p></GlassCard>
      </div>
    );
  }
  ...
  return (
    <div className="w-full max-w-md px-5 pt-8 pb-16 flex-1">
      <div className="flex items-center gap-2.5 mb-5">
        <Logo size={9} />
        <div><p className="text-xs" style={{ color: "#9CA3AF" }}>{maestra.funcion}</p><p className="font-semibold" style={{ color: "#1F3A35" }}>{maestra.name} · {maestra.grupo}</p></div>
      </div>
      ...
      <GlassCard className="p-4 mb-4">
        <p className="text-xs font-medium mb-2" style={{ color: "#2E7D74" }}>Registro rápido para todo el grupo</p>
        <div className="grid grid-cols-3 gap-1.5 mb-1.5">
          {MEAL_OPTIONS.map((o) => (
            <button key={o.key} disabled={bulkBusy} onClick={() => bulkMeal(o.key)} className="py-2.5 rounded-xl text-xs font-medium disabled:opacity-40" style={{ background: "rgba(95,165,90,0.1)", color: "#3F7A45" }}>{o.label}</button>
          ))}
        </div>
        <p className="text-xs mb-1.5" style={{ color: "#9CA3AF" }}>Siesta (todos)</p>
        <div className="grid grid-cols-2 gap-1.5 mb-1.5">
          {NAP_DURATIONS.map((d) => (
            <button key={d.key} disabled={bulkBusy} onClick={() => bulkNap("si", d.key)} className="py-2.5 rounded-xl text-xs font-medium disabled:opacity-40" style={{ background: "rgba(1,180,188,0.08)", color: "#0E7C82" }}>{d.label}</button>
          ))}
        </div>
        <button disabled={bulkBusy} onClick={() => bulkNap("no")} className="w-full py-2.5 rounded-xl text-xs font-medium disabled:opacity-40" style={{ background: "rgba(255,255,255,0.7)", color: "#4B5563", border: "1px solid rgba(107,114,128,0.2)" }}>No durmieron siesta (todos)</button>
      </GlassCard>
      ...
      <button onClick={onExitDemo} className="text-xs mt-6 mx-auto flex items-center gap-1 justify-center w-full" style={{ color: "#9FC6BE" }}>
