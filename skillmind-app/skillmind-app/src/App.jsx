            ...
            {!readOnly && (
              napAsking ? (
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-1.5">
                    {NAP_DURATIONS.map((d) => (
                      <button key={d.key} onClick={() => logNapYes(d.key)} className="py-2.5 rounded-xl text-xs font-medium" style={{ background: "rgba(91,95,239,0.08)", color: "#3730A3" }}>{d.label}</button>
                    ))}
                  </div>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-1.5">
                    <button onClick={() => setNapAsking(true)} className="py-2.5 rounded-xl text-xs font-medium" style={{ background: "rgba(20,184,166,0.1)", color: "#0D9488" }}>Sí</button>
  ...
  return (
    <GlassCard className="p-4">
      <button onClick={() => setOpen((v) => !v)} className="w-full flex items-center justify-between gap-3">
        <div className="min-w-0 text-left"><p className="text-sm font-medium" style={{ color: "#1E1B4B" }}>{m.name}</p><p className="text-xs" style={{ color: "#9CA3AF" }}>{m.grupo} · {grupoNinos.length} niño(s)</p></div>
        <ChevronDown size={16} color="#C7C9E8" className="shrink-0 transition-transform" style={{ transform: open ? "rotate(180deg)" : "none" }} />
      </button>
      {open && (
        <div className="mt-3 pt-3 space-y-3" style={{ borderTop: "1px solid rgba(91,95,239,0.1)" }}>
          <div className="flex items-center gap-2">
            <button onClick={() => copyLink(m.code)} className="flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-xl text-xs font-medium" style={{ background: "rgba(91,95,239,0.08)", color: "#3730A3" }}><Copy size={13} /> Copiar link</button>
            <button onClick={() => onPreview(m.code)} className="flex-1 py-2.5 rounded-xl text-xs font-medium" style={{ background: "rgba(255,255,255,0.7)", color: "#4C4E9E", border: "1px solid rgba(91,95,239,0.15)" }}>Vista previa</button>
          </div>
    ...
    <div className="w-full max-w-md px-5 pt-8 pb-16 flex-1">
      ...
      <GlassCard className="p-4 mb-4">
        ...
        <div className="grid grid-cols-2 gap-1.5 mb-1.5">
          {NAP_DURATIONS.map((d) => (
            <button key={d.key} disabled={bulkBusy} onClick={() => bulkNap("si", d.key)} className="py-2.5 rounded-xl text-xs font-medium disabled:opacity-40" style={{ background: "rgba(91,95,239,0.08)", color: "#3730A3" }}>{d.label}</button>
          ))}
        </div>
